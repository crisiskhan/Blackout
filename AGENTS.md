...
- WALK is the shortest walkable street path. DRIVE is the fastest packed street path
  (road class on the graph, one-ways bind cars). WALK, DRIVE, and SPEAK say Walk or Drive
  turn by turn with the live voice, name the streets they turn onto when the pack has them,
  and never paint that script on the field. SPEAK replays. INSTRUMENTS VOICE picks
  STEEL / NIGHT / RANGE / MESH / DESERT.
...
`test_walkable_next_pack.py`, `test_states_pack.py`, `test_tx_west_style.py`, `test_voice_nav.py`,
