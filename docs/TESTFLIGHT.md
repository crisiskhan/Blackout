...
- `33925258357` (`4456206`): KEEP Dist cert reuse + Watch omitted. Hand-zip IPA ready. `altool` −19000 on **`com.maplibre.mapbox`** (MapLibre.framework Info.plist). Vendor slices now use `com.crisiskhan.blackout.maplibre`. `tf-archive.sh` fails closed if the old id is still in the IPA. Do not create an ASC app for MapLibre.
