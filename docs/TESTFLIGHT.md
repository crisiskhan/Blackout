...
## HUD 120 Internal (Crisis GO)

HUD tree `1b880e8` tagged `tf-120`. Run [34788757841](https://github.com/crisiskhan/Blackout/actions/runs/34788757841) in flight. KEEP `45YLWHL6UP` untouched. Tag 120 mints max ASC + 1 (expected **113** if 112 is still the tip). This CPV will not be numbered 120. IPA 120 is FIELD SEARCH live ASK (`ASK · LIVE`), open-card layout (VISION stays on SEARCH, NEXT after DO), FieldAsk device compile, and the MeshDTN simulator leftover (`split(separator:)`). Watch omitted. Internal only. Do not re-upload 54–112. Do not retag 85–120. Tree CPV stays 1. Leave **112** until TestFlight shows the new CPV Ready, then install that `0.1.0`. Do not wait for a build numbered 120.
