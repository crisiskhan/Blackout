...
- `33929367958` (`0e3d31d`): signing + archive + IPA succeeded (Local Dist `FG5MXH5347`, CPV 54). `altool` −19000 on **`com.crisiskhan.blackout.maplibre`**. Renaming the nested FMWK under the owner prefix is not enough — altool wants an ASC application record for every unique `CFBundleIdentifier`. Nested FMWK must use the parent app id `com.crisiskhan.blackout`. Still do not create an ASC app for MapLibre.
...
- App target now links **MapLibreMap only** (MapLibreMap already depends on the vendor binary). `tf-archive.sh` runs `tools/tf_ipa_inspect.py` after the IPA exists: assert app/widget BIDs, rewrite any nested FMWK whose BID is not the parent app id to `com.crisiskhan.blackout`, fail closed, re-zip if rewritten.
