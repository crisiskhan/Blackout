...
- `33930228429` (`31fcfe5`): Dist prune + iOS Local replace succeeded (`KR7N96LT42`). Widget Local CREATE then ASC HTTP 500 `UNEXPECTED_ERROR`. Dist revoke leaves the previous Local profile `INVALID`; an ACTIVE-only list misses it so CREATE collides. List `ACTIVE,INVALID`. Retry Local profile CREATE on 500. Never delete KEEP-named profiles.
