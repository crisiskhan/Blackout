...
- App target now links **MapLibreMap only** (MapLibreMap already depends on the vendor binary). `tf-archive.sh` runs `tools/tf_ipa_inspect.py` after the IPA exists: assert app/widget BIDs, strip nested FMWK `CFBundleIdentifier` (keep `FMWK`), fail closed on foreign `Payload/*.app` / `PlugIns/*.appex`, re-zip if rewritten. Upload uses `xcrun altool --upload-package` with `--apple-id "$ASC_APP_ID"` `--bundle-id com.crisiskhan.blackout` `--bundle-version` `--bundle-short-version-string`.
...
- `33931034850` (`f4ba3af`): signing + archive + hand-zip IPA succeeded (Local Dist `FQK7KQDXGP`, CPV 54, inspect no FMWK rewrite). altool bound the primary app, then 409: `Payload/Blackout.app` is not signed using an Apple submission certificate. ExportOptions asked for Apple Distribution while the mint was `IOS_DISTRIBUTION`; exportArchive looked for `3rd Party Mac Developer Installer`. Hand-zip used info-zip. Mint **Apple Distribution** first, set ExportOptions from `DIST_CERT_TYPE`, write xcarchive `SigningIdentity`, package the IPA with `ditto -c -k`, `codesign --verify --deep --strict` before upload.
