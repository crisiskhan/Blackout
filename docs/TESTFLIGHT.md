...
- `33927056130` (`ce331dd`): mint worked (`HAS_LOCAL_DIST_KEY=1`). Archive still exit 65 because CLI `CODE_SIGN_IDENTITY="iPhone Distribution"` applied to **every** target, including SPM packages (`VisionCoreML`, `Tokens`, `MapLibreMap`) that stay Automatic: “automatically signed for development, but a conflicting code signing identity iPhone Distribution has been manually specified.” Do **not** pass `CODE_SIGN_IDENTITY` on the `xcodebuild archive` command line. Keep the per-target CI pbx patch (Manual + Dist identity + Local profile) on app/widget only.
