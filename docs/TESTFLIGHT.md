...
- INFERENCE: archive packaging (`IDEArchivedApplicationErrorDomain`) reads the built `.app` Info.plist for `ApplicationProperties.CFBundleIdentifier` and found it empty. `GENERATE_INFOPLIST_FILE` did not pass a generated-info additional file into `infoPlistUtility`.
- Project fix: `Blackout/Info.plist` now sets `CFBundleIdentifier=$(PRODUCT_BUNDLE_IDENTIFIER)` plus version/executable keys so ProcessInfoPlistFile expands them into the `.app`. CI logs the processed identifier and injects it if still missing, then snapshots the `.app` for hand-zip if archive packaging still fails. Do not delete `PrivacyInfo` / `Assets.car` / AppIcons.
- `33829001016` (`43f13b9`): processed `CFBundleIdentifier=com.crisiskhan.blackout` / version 54. Snapshot ran. Archive packaging still exited 70, but left `Blackout.xcarchive/Products/Applications/Blackout.app` + dSYMs + Signatures and **no** xcarchive `Info.plist`. Recover overwrote the snapshot with that already-signed product, `rm -rf _CodeSignature`, then `codesign` reported `bundle format unrecognized`. Do not re-seal a signed archive product. Write the missing xcarchive `Info.plist` and `exportArchive` or hand-zip the signed `.app` as-is.
