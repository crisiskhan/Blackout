...
- `34745805359` (`adfb764`): HUD tag `tf-99`. macos-15 / Xcode 26.3. `HAS_LOCAL_DIST_KEY=1`. `exportArchive OK`. CPV **99**. Dist `66KB44VS8G` (`DISTRIBUTION`). KEEP `45YLWHL6UP` untouched. altool `UPLOAD SUCCEEDED with no errors`. `WAIT no build 99`, then ASC `960c7bd3-fe7e-47a2-a5dc-55c53ae9db55` **VALID**. PATCH 200. `ASSIGN Internal 99 204`. `usesNonExemptEncryption: null`. Live instruments, SPEAK street vertices, unique leftover Holds. Historical — scoring tip is **115**.
...
- `34752403614` (`9c33aa8`): HUD tag `tf-115`. macos-15 / Xcode 26.3. `HAS_LOCAL_DIST_KEY=1`. `exportArchive OK`. CPV **108**. Dist `8CWLXNF8C7` (`DISTRIBUTION`). KEEP `45YLWHL6UP` untouched. altool uploaded CFBundleVersion 108. `WAIT no build 108`, then ASC `0315a666-fda5-4fed-924e-fdd1f47cdc2b` **VALID**. PATCH 200. `ASSIGN Internal 108 204`. `usesNonExemptEncryption: null`. Tag 115 is not TestFlight 115 — tags 101–109 minted CPV 100–107. Scoring IPA is `0.1.0 (108)`. Leave 109 until TestFlight shows 108 Ready.
