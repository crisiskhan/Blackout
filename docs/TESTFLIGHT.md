...
- `33926435868` (`e096955`): KEEP reuse, `HAS_LOCAL_DIST_KEY=0`, no Manual patch. `xcodebuild archive` then asked for **Apple Development** certs/profiles (`Revoke certificate` / no iOS App Development profiles) and exited 65 — no IPA. Pin `CODE_SIGN_IDENTITY=iPhone Distribution` (Automatic + AuthKey) so archive does not touch Development certs. Do not revoke KEEP. Do not create a Dist cert when KEEP exists.
