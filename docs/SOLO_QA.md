...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood) and this pack's animals as range (javelina / coyote / black bear), not a meal, not a pin. TX EAST (if switched in INSTRUMENTS) names cottonmouth / copperhead, not javelina. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that state's tree-use card (the trees this cover is), then `NEXT · PLANT` (plant-danger). Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
