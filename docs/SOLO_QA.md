...
- [ ] PASS / FAIL / N/A — Hold a silver peak or sinkhole mark (circle, no class label) → Peak or Cave or hole. TX WEST peak is `FIELD · ANIMAL` then `NEXT · COLD` (Texas has no ice-on-rock card). Hole is `FIELD · CAVE` then `NEXT · COLD`. `BOOK` on the hole is `CAVE · COLD`. TX WEST sinkhole is 31.694905, −106.441133. Do not go in alone. No tourist cave guide.
