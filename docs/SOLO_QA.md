...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood), not a meal, and says Field has this pack's tree and animal cards. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that state's tree-use card (the trees this cover is), then `NEXT · PLANT` (plant-danger). Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
...
- [ ] PASS / FAIL / N/A — Hold a silver peak or sinkhole mark (circle, no class label) → Peak or Cave or hole. TX WEST peak is `FIELD · ANIMAL` then `NEXT · COLD` (Texas has no ice-on-rock card). Hole is `FIELD · CAVE`. TX WEST sinkhole is 31.694905, −106.441133. Do not go in alone. No tourist cave guide.
