...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood) and this pack's animals as range (javelina / coyote / black bear), not a meal, not a pin. TX EAST (if switched in INSTRUMENTS) names cottonmouth / copperhead, not javelina. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that pack's tree-use card (TX WEST `tx-tree-use` with mesquite; TX EAST `tx-east-tree-use` live oak / pecan / cedar elm, not mesquite). Then `NEXT · PLANT` (plant-danger). Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
...
- [ ] PASS / FAIL / N/A — VISION still opens this pack's cards: mammal → `FIELD · ANIMAL`, tree → `FIELD · PLANT`, cactus → `FIELD · PLANT`, snake → `FIELD · BITE`, fungi → `FIELD · FUNGI`. TX EAST `kind:mammal` opens `tx-east-mammal` (coyote / deer, not javelina). A `tx-javelina` still still opens `tx-mammal`. UNKNOWN and `NO VISION MODEL` do not invent a card. Same Field book the hold uses. Never edible.
