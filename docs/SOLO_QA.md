...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood / loblolly pine) and this pack's animals as range (javelina / coyote / hog / black bear), not a meal, not a pin. TX EAST (if switched in INSTRUMENTS) names cottonmouth / copperhead and hog, not javelina. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that pack's tree-use card (TX WEST `tx-tree-use` with mesquite; TX EAST `tx-east-tree-use` live oak / pecan / cedar elm / loblolly pine, not mesquite). Then `NEXT · PLANT` (plant-danger). Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
