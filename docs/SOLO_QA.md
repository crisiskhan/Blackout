...
- [ ] PASS / FAIL / N/A — Hold desert scrub → Desert scrub. DO names this pack's snakes and mammals as range (diamondback / javelina, not a pin). `FIELD · BITE` opens that state's snake card. Last step becomes `NEXT · ANIMAL`, then tree-use / `NEXT · BITE` / heat. No animal icon on the canvas.
