...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood), not a meal, and says Field has this pack's tree and animal cards. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that state's plant-danger card. Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Last step becomes `NEXT · PLANT` (tree-use), then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
