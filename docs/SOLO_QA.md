...
- [ ] PASS / FAIL / N/A — Hold woodland / bosque / park → Woodland, Bosque or Park. DO names this pack's trees (mesquite / cottonwood / loblolly pine) and this pack's animals as range (javelina / coyote / hog / black bear), not a meal, not a pin. TX EAST woodland / park names hog, not javelina, not cottonmouth on every park. TX EAST bosque names cottonmouth; `FIELD · PLANT` still opens `tx-east-tree-use`, and the walk includes `tx-east-snake` (cottonmouth treatment), not the west diamondback card. `BOOK` reads `PLANT · ANIMAL · FOOD · BITE · SHELTER · FUNGI`. Button `FIELD · PLANT` opens that pack's tree-use card (TX WEST `tx-tree-use` title West Texas trees, mesquite and cottonwood; TX EAST `tx-east-tree-use` live oak / pecan / cedar elm / loblolly pine / cottonwood, not mesquite; NM `nm-tree-use` names Rio Grande cottonwood, juniper, piñon, aspen). SPEAK on that tree card names the same trees (NM: aspen is high country). Then `NEXT · PLANT` (plant-danger). Header `CARD 1 OF` the trail (not `STEP 1 OF 1` on every card). Then `NEXT · ANIMAL`, `NEXT · SHELTER`, `NEXT · FUNGI`, `NEXT · FOOD`. Never edible.
- [ ] PASS / FAIL / N/A — Hold a silver glasshouse outline (walking zoom; no class label). Klass Glasshouse. Hold DO names oleander (TX) or datura (NM), same as plant-danger SPEAK. `FIELD · PLANT` opens `tx-plant-danger` (pretty is not food), not tree-use, not plant-use. SPEAK names oleander. Never edible. No animal icon. TX EAST named `Vickery Wholesale Greenhouse` 30.313936, −97.617562. TX WEST unnamed 32.502967, −106.933833. Farmland still Klass Irrigated ground. Not javelina country. `FIELD · PLANT` on a field opens tree-use, not a hunt.
- [ ] PASS / FAIL / N/A — Hold a botanic garden or conservatory (silver outline at walking zoom, not Conservatory At North Austin). Klass Botanic garden. Hold DO names oleander (TX) or datura (NM), not jumping cholla. `FIELD · PLANT` opens plant-danger (pretty is not food), not tree-use, not plant-use, not cactus. TX SPEAK names oleander. NM plant-danger SPEAK names datura, not jumping cholla. Never edible. NM `Albuquerque BioPark Botanic Garden` 35.094694, −106.682101. NM `Barelas Community Garden` 35.078118, −106.653090. TX WEST `Rose Garden`. `Conservatory At North Austin` stays Built-up ground. `Cactus Point Park` stays Park. `Moontower Saloon Beer Garden` stays Park.
- [ ] PASS / FAIL / N/A — Hold a cactus or desert garden (silver outline at walking zoom). Klass `Cactus garden`. `FIELD · PLANT` opens `tx-cactus` (NM `nm-cactus`). Hold DO and the cactus step name prickly pear and yucca (NM: cholla, yucca, sotol). Spines, not a meal. Walk does not open plant-danger. Not oleander. TX WEST `Three Crosses Cactus Garden` 32.332056, −106.782048. TX WEST `Chihuahuan Desert Conservatory` 33.150801, −107.238341. TX WEST `Desert Garden Park`. `Parque Cactus del Desierto` stays Park.
- [ ] PASS / FAIL / N/A — Hold desert scrub → Desert scrub. DO names this pack's snakes, mammals, yucca and prickly pear as range, not a pin. TX WEST diamondback and javelina. TX EAST cottonmouth / copperhead and hog. NM Prairie rattler or diamondback. `BOOK` reads `BITE · ANIMAL · PLANT · FOOD · HEAT`. `FIELD · BITE` opens that state's snake card. SPEAK names that pack's viper. Last step becomes `NEXT · ANIMAL`, then cactus / tree-use / `NEXT · BITE` / `NEXT · FOOD`. No animal icon on the canvas.
- [ ] PASS / FAIL / N/A — Hold a silver peak or sinkhole mark (circle, no class label) → Peak or Cave or hole. TX WEST peak is `FIELD · ANIMAL` then `NEXT · BITE` then `NEXT · COLD` (Texas has no ice-on-rock card). Peak walk includes bite treatment. `BOOK` on the peak is `ANIMAL · BITE · COLD`. DO names coyote and deer range, not ice. Hole is `FIELD · CAVE` then `NEXT · COLD`. SPEAK names a hole, dark, still air, cold. Do not go in alone. `BOOK` on the hole is `CAVE · COLD`. TX WEST sinkhole is 31.694905, −106.441133. No tourist cave guide.
- [ ] PASS / FAIL / N/A — Hold a cave preserve or named sink (silver outline at walking zoom, not Bee Cave). Klass Cave or hole. `FIELD · CAVE` then `NEXT · COLD`. TX EAST `Discovery Well Cave Preserve` 30.490167, −97.856308. TX EAST `Blowing Sink` 30.193035, −97.850443 is a named sink tagged wetland — not bosque, not cottonmouth. `Bee Cave Central Park` stays Park / `FIELD · PLANT`. Never edible. No tourist cave guide.
- [ ] PASS / FAIL / N/A — Hold a wildlife management area (silver outline at walking zoom, not Wildlife Drive). Klass Wildlife range. `FIELD · ANIMAL`. `BOOK` reads `ANIMAL · BITE · FOOD · PLANT` (animals first, not picnic woodland). Range, not a pin. West names javelina, coyote, deer, and diamondback as range. East names hog and cottonmouth, not javelina. NM names mule deer with bear, elk, and prairie rattler or diamondback. FIELD mammal step names that deer — give it the road. NM mammal SPEAK names the bite card. Hold DO on wildlife range names the bite card. `NEXT · BITE` is that pack's snake card; SPEAK names that viper. FIELD food step names javelina / hog / elk — already have, not a hunt. NM `Marquez Wildlife Management Area` 35.327562, −107.319389. NM `Whitfield Wildlife Conservation Area` 34.667597, −106.738110. NM `State Game Commission Land` 34.566661, −106.758850. NM `Rio Grande Nature Center State Park` 35.129587, −106.683343. TX EAST `Colorado River Park Wildlife Sanctuary` 30.246316, −97.694742. TX EAST `Wild Basin Wilderness Preserve` 30.312988, −97.820186. TX EAST `Barrow Nature Preserve` 30.371197, −97.769187. TX EAST `Stillhouse Hollow Nature Preserve` 30.371484, −97.763507. TX EAST `Big Walnut Creek Nature Preserve` 30.326309, −97.650016. TX EAST `Bright Leaf Natural Area` 30.335604, −97.770799. `Wildlife Drive Park` stays Park / `FIELD · PLANT`. `New Mexico Department of Game & Fish` stays Park. `Open Space Visitor Center` stays Park. `Godzilla Preserve` / `Whitestone Preserve` / `Westside Preserve` stay Park. `Wilderness Gate` stays Built-up ground. Never edible. No animal icon.
- [ ] PASS / FAIL / N/A — Hold an open reserve (silver outline at walking zoom). Klass Open reserve. `FIELD · BITE` then cactus / animal / tree-use — not picnic woodland tree-use first. `BOOK` reads `BITE · ANIMAL · PLANT · FOOD · HEAT`. TX WEST diamondback. NM prairie rattler or diamondback. TX EAST cottonmouth / copperhead. SPEAK names that pack's viper. Pronoun Cave ACEC stays Cave or hole / `FIELD · CAVE`. TX WEST `Alamo Mountain Area of Critical Environmental Concern` 32.032184, −105.636055. NM `Jones Canyon Area of Critical Environmental Concern` 35.848354, −107.023921. TX EAST `Decker Tallgrass Prairie Preserve` 30.294288, −97.603891. `Prairie Hills` stays Built-up. `Gracecus Way` stays a road. Never edible. No animal icon.
