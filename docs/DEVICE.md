...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - DEST + WALK, then SPEAK. The banner lists the turns one per row inside its own scrolling box; it never cuts a word and never eats the canvas.
    - Field chrome is at most two lines: one status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`, deduped) and one `DEST … · BEARING …` line. A bare `TRUE`, a doubled `OFF GRAPH`, or a standalone BEARING row is a FAIL.
