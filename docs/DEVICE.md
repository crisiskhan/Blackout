...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - Field chrome is at most three short lines: one deduped status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`), one dest row of two 44pt chips (`BEARING` accent-red heartbeat glow / `COORDINATES` GNSS-green heartbeat glow) when there is somewhere to walk (dest, drawn route, or LOCK-ON), even without heading. Tap one — the dest line under the chips shows that readout (`45°` / `NO HEADING`, or live GNSS `31.76190, -106.49000` / `NO FIX`). Never a DEST pair. Idle MAP has no dest chips. One Speak status line. A bare `TRUE`, a doubled `OFF GRAPH`, dest chips with no dest, or a Speak line left over from an old destination is a FAIL.
