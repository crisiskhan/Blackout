...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - DEST + WALK (or DEST + DRIVE). WALK and DRIVE speak the turn-by-turn with the live INSTRUMENTS voice (STEEL / NIGHT / RANGE / MESH / DESERT). SPEAK replays the same script. The phone says Walk / Drive / Turn / Arrive, names the street it is on and the street it turns onto when the pack has those names, and the route stays drawn on the map. DRIVE follows the faster packed street path, not a residential maze. A glass TURNS plate lists HUD rows (`LEFT · MONTANA AVENUE`). The field gets one short line only — `SPEAK · 3 TURNS · 984 FT`. Dest rail `TURNS` names the next street. The orange walk-script paragraph from the tip-67 stills must be gone; any multi-line text over the canvas is a FAIL.
