...
8. BEFORE tapping ACTIVATE: iPhone Control Center — Airplane Mode ON, then Bluetooth ON. Wi-Fi stays off. Cell stays off. The ACTIVATE field is the compass field poster over the pack map; streets show through the knocked black. The compass fills the well — same size as the cold splash (the square mark on void), not a small chip on a movie poster. The square mark still sits on the ring. Wait until ACTIVATE is live, then tap it. It opens MAP. Then score `docs/SOLO_QA.md`. For the next MAP still, score only the tip-60 bar in that file: full-height canvas, pack outline+puck, single MARK, no CALL SOS on browse MAP, no solid-red slab.
...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - DEST + WALK, then SPEAK. Speak is **voice plus the silver route line**: the phone says the whole turn-by-turn out loud (Walk / Turn / Arrive), names the street it is on and the street it turns onto when the pack has those names, and the route stays drawn on the map. Repeat DEST + DRIVE, then SPEAK — the script says Drive, not Walk, and DRIVE follows the faster packed street path, not a residential maze. A glass TURNS plate lists HUD rows (`LEFT · MONTANA AVENUE`). The field gets one short line only — `SPEAK · 3 TURNS · 984 FT`. Dest rail `TURNS` names the next street. The orange walk-script paragraph from the tip-67 stills must be gone; any multi-line text over the canvas is a FAIL. INSTRUMENTS VOICE is STEEL / NIGHT / RANGE / MESH / DESERT.
