...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    - HUD overlay: search on its own row; `INSTRUMENTS` + `LOCK-ON`/`LOCKED` + `EYE` wrap under it; `MARK` `WALK` `DRIVE` `SPEAK` as a four-cell dock at the thumb. No `INST`, no `INSTRUME…`. INSTRUMENTS opens the instruments sheet (RULER / USNG / MAG/TRUE live there; `EYE` sits next to NIGHT / SUN). SPEAK is on the dock. `EYE` frames the party + last marks + water, north-up, on the packed area. `EYE` holds satellite range over the framed desk. Heading stays north-up. Thumb pan stays on the pack. Two-finger tilt stays on the pack. EYE drops LOCK-ON. LOCK-ON and EYE are never both live. `EYE` leaves to the puck. The pack sits clear of the HUD.
