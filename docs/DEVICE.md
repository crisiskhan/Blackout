...
8. BEFORE tapping ACTIVATE: iPhone Control Center — Airplane Mode ON, then Bluetooth ON. Wi-Fi stays off. Cell stays off. The ACTIVATE field is the compass field poster over the pack map; streets show through the knocked black. The square mark still sits on the ring. Wait until ACTIVATE is live, then tap it. It opens MAP. Then score `docs/SOLO_QA.md`. For the next MAP still, score only the tip-60 bar in that file: full-height canvas, pack outline+puck, single MARK, no CALL SOS on browse MAP, no solid-red slab.
...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - Field chrome is at most three short lines: one deduped status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`), one COORDINATES row with a 44pt `COORDINATES` chip (GNSS-green heartbeat glow) on void — no grey plate. Idle MAP with live GNSS prints YOU (`31.76190, -106.49000`). With a dest, the same chip prints the pin being walked to. No MAP BEARING chip. Never a DEST pair. LOCK-ON or a drawn route without a dest does not invent dest coords. No GNSS and no dest stays quiet. One Speak status line. A bare `TRUE`, a doubled `OFF GRAPH`, dest coords with no dest, or a Speak line left over from an old destination is a FAIL.
