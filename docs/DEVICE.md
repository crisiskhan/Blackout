...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - DEST + WALK, then SPEAK. Speak is **voice plus the silver route line**: the phone says the whole turn-by-turn out loud and the route stays drawn on the map. The field gets one short line only — `SPEAK · 3 TURNS · 984 FT`. The orange walk-script paragraph from the tip-67 stills must be gone; any multi-line text over the canvas is a FAIL.
