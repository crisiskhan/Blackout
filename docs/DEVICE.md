...
9. After the next Internal (HUD chrome, not this commit): on MAP, the canvas is full-bleed under a four-cell dock (MARK / WALK / DRIVE / SPEAK) and overlay INSTRUMENTS / LOCK-ON / GODS EYE (whole words, wrapping). RULER / USNG / MAG/TRUE are in INSTRUMENTS. Canvas has no OSM credit, no MapLibre mark, and no style.json / MapKit debug overlay. Tap a street away from the puck (or a search hit). Confirm DEST. Tap **WALK** — a silver line should follow streets from YOU to DEST. Repeat **DRIVE**. WALK/DRIVE are never disabled; if the graph cannot route, chrome `OFF GRAPH` (no fake line). MARK is one row per coord. Road names stay at walking zoom.
...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    - HUD overlay: search on its own row; `INSTRUMENTS` + `LOCK-ON`/`LOCKED` + `GODS EYE` wrap under it; `MARK` `WALK` `DRIVE` `SPEAK` as a four-cell dock at the thumb. No `INST`, no `INSTRUME…`. INSTRUMENTS opens the instruments sheet (RULER / USNG / MAG/TRUE live there). SPEAK is on the dock. GODS EYE fits the pack. GODS EYE is an overhead fly over the packed area. GODS EYE holds satellite range over the packed area. Two-finger orbit stays on the pack. Thumb pan stays on the pack. Two-finger tilt stays on the pack. GODS EYE drops LOCK-ON. LOCK-ON and GODS EYE are never both live. GODS EYE leaves pinch. The pack sits clear of the HUD.
