...
9. After the next Internal (HUD chrome, not this commit): on MAP, the canvas is full-bleed under a four-cell dock (MARK / WALK / DRIVE / SPEAK) and overlay INST / LOCK. RULER / USNG / MAG/TRUE are in INST. Canvas has OSM credit and no style.json / MapKit debug overlay. Tap a street away from the puck (or a search hit). Confirm DEST. Tap **WALK** — a cyan line should follow streets from YOU to DEST. Repeat **DRIVE**. WALK/DRIVE are never disabled; if the graph cannot route, chrome `OFF GRAPH` (no fake line). MARK is one row per coord. Road names stay at walking zoom.
10. NM (Albuquerque / Sandia) and TX EAST (Austin / Lost Pines) are walkable catalog packs. On MAP → INST → PACKS, tap **NM** or **TX EAST** to open them. Streets, names, and a local WALK/DRIVE graph should be present at walking zoom with © OpenStreetMap. TX WEST stays default first-open until Crisis says switch.
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    - HUD overlay: search + `INST` + `LOCK`/`LOCKED` at the top; `MARK` `WALK` `DRIVE` `SPEAK` as a four-cell dock at the thumb. No `INSTRUME…`. INST opens the instruments sheet (RULER / USNG / MAG/TRUE live there). SPEAK is on the dock.
