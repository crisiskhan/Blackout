...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    - HUD overlay: search on its own row; `INSTRUMENTS` + `LOCK-ON`/`LOCKED` + `GODS EYE` wrap under it; `MARK` `WALK` `DRIVE` `SPEAK` as a four-cell dock at the thumb. No `INST`, no `INSTRUME…`. INSTRUMENTS opens the instruments sheet (RULER / USNG / MAG/TRUE live there). SPEAK is on the dock. GODS EYE fits the pack. GODS EYE is an overhead fly over the packed area. Two-finger orbit stays on the pack. GODS EYE drops LOCK-ON. LOCK-ON and GODS EYE are never both live. GODS EYE leaves pinch. The pack sits clear of the HUD.
