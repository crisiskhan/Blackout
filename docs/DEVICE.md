...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - Field chrome is at most three short lines: one deduped status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`), one dest row with a 44pt `COORDINATES` chip (GNSS-green heartbeat glow) when there is a destination. The dest line is the location being traveled to (`31.76190, -106.49000`). No MAP BEARING chip. Never a DEST pair. LOCK-ON or a drawn route without a dest does not invent dest coords. Idle MAP has no dest chips. One Speak status line. A bare `TRUE`, a doubled `OFF GRAPH`, dest chips with no dest, or a Speak line left over from an old destination is a FAIL.
