...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - Field chrome is at most three short lines: one deduped status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`), one `BEARING <deg>°` line when there is somewhere to walk (dest, drawn route, or LOCK-ON) that also prints YOU as `31.76190, -106.49000` when GNSS has a live fix (never a DEST pair; no fix stays `BEARING <deg>°` only), one Speak status line. A bare `TRUE`, a doubled `OFF GRAPH`, a `BEARING` with no dest, or a Speak line left over from an old destination is a FAIL.
