...
8. BEFORE tapping ACTIVATE: iPhone Control Center — Airplane Mode ON, then Bluetooth ON. Wi-Fi stays off. Cell stays off. The boot screen is the logo, not a pack menu. Wait until ACTIVATE is live, then tap it. Then score `docs/SOLO_QA.md`. For the next MAP still, score only the tip-60 bar in that file: full-height canvas, pack outline+puck, single MARK, no CALL SOS on browse MAP, no solid-red slab.
9. After the next Internal (HUD chrome, not this commit): on MAP, the canvas is full-bleed under a four-cell dock (MARK / WALK / DRIVE / SPEAK) and overlay INSTRUMENTS / LOCK-ON (whole words, wrapping). RULER / USNG / MAG/TRUE are in INSTRUMENTS. Canvas has OSM credit and no style.json / MapKit debug overlay. Tap a street away from the puck (or a search hit). Confirm DEST. Tap **WALK** — a cyan line should follow streets from YOU to DEST. Repeat **DRIVE**. WALK/DRIVE are never disabled; if the graph cannot route, chrome `OFF GRAPH` (no fake line). MARK is one row per coord. Road names stay at walking zoom.
10. NM (Albuquerque / Sandia) and TX EAST (Austin / Lost Pines) are walkable catalog packs. On MAP → INSTRUMENTS → PACKS, tap **NM** or **TX EAST** to open them. Streets, names, and a local WALK/DRIVE graph should be present at walking zoom with © OpenStreetMap. TX WEST stays default first-open until Crisis says switch.
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    ...
    - Field chrome is at most three short lines: one deduped status line (`OFF GRAPH` / `TRUE NORTH` / `RULER …`), one `BEARING <deg>°` line when there is somewhere to walk (dest, drawn route, or LOCK-ON), one Speak status line. A bare `TRUE`, a doubled `OFF GRAPH`, a `BEARING` with no dest, or a Speak line left over from an old destination is a FAIL.
