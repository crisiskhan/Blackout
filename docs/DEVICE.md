...
9. After the next Internal (HUD chrome, not this commit): on MAP, the canvas is full-bleed under a four-cell dock (MARK / WALK / DRIVE / SPEAK) and overlay INSTRUMENTS / LOCK-ON / `EYE` (whole words, wrapping). RULER / USNG / MAG/TRUE are in INSTRUMENTS. Walking MAP has no OSM credit, no MapLibre mark, and no style.json / MapKit debug overlay. `EYE` stamps © OpenStreetMap contributors. Tap a street away from the puck (or a search hit). Confirm DEST. Tap **WALK** — a silver line should follow streets from YOU to DEST. Repeat **DRIVE**. WALK/DRIVE are never disabled; if the graph cannot route, chrome `OFF GRAPH` (no fake line). MARK is one row per coord. Road names stay at walking zoom.
...
11. Tip 68 (Speak finish + clean field). Score these on the MAP still:
    - HUD overlay: search on its own row; `INSTRUMENTS` + `LOCK-ON`/`LOCKED` + `EYE` wrap under it; `MARK` `WALK` `DRIVE` `SPEAK` as a four-cell dock at the thumb. No `INST`, no `INSTRUME…`. INSTRUMENTS opens the instruments sheet (RULER / USNG / MAG/TRUE live there; `EYE` sits next to NIGHT / SUN). SPEAK is on the dock. `EYE` is a standalone 3D satellite desk: frames the party + last marks + water, enters north-up, on the packed area. `EYE` holds satellite range over the framed desk. Walking SEARCH recedes. Heading starts north-up. Thumb pan stays on the pack. Two-finger tilt stays on the pack. Two-finger orbit stays on the pack. EYE drops LOCK-ON. LOCK-ON and EYE are never both live. `EYE` leaves to the puck. The pack sits clear of the HUD.
