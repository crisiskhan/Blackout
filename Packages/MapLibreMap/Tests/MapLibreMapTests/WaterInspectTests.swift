...
final class WaterInspectTests: XCTestCase {
    ...
    func testAStillFingerPastTheClockAsksAboutTheGround() {
        XCTAssertEqual(
            InspectGesture.verdict(elapsedSeconds: Inspect.holdSeconds + 0.06, movedPoints: 0),
            .inspect
        )
    }
    ...
    func testTheClockAloneIsNotEnough() {
        XCTAssertEqual(
            InspectGesture.verdict(elapsedSeconds: Inspect.holdSeconds - 0.01, movedPoints: 0),
            .waiting
        )
    }
    ...
    func testTheShippedIndexLoadsAndHoldsTheWaterTheToolCounted() throws {
        for (pack, records) in [("tx-west", 19_625), ("tx-east", 19_504), ("nm", 14_792)] {
    ...
    func testASilentTankStaysLeaveEvenWhenTheIndexFindsWaterNearby() throws {
        let index = try XCTUnwrap(WaterIndex.load(try shippedIndexData("tx-west")))
        let card = Inspect.resolve(
        ...
        XCTAssertEqual(card.advice, .leave)
        XCTAssertEqual(card.klass, "Tank, contents unrecorded")
    }

    func testEmptyGroundNamesTheNearestWater() throws {
        let index = try XCTUnwrap(WaterIndex.load(try shippedIndexData("tx-west")))
        let card = Inspect.resolve(
            tags: [:],
            lat: 31.76,
        ...
        let said = (card.klass + " " + card.why + " " + card.doLine).lowercased()
        XCTAssertTrue(
            card.kind == .water || said.contains("nearest water"),
            card.why
        )
    }

    func testADowntownStreetIsNotStolenByWaterTwentyMetresOff() throws {
        let press = (lat: 31.7600, lon: -106.4900)
        let twentyNorth = press.lat + 20.0 / 111_320.0
        let index = try XCTUnwrap(tankIndex(lat: twentyNorth, lon: press.lon))
        let card = Inspect.resolve(
            tags: ["highway": "residential", "name": "Oregon St"],
            lat: press.lat,
            lon: press.lon,
            zoom: 16,
            index: index
        )
        XCTAssertEqual(card.kind, .street)
        XCTAssertEqual(card.klass, "Road")
        XCTAssertEqual(card.title, "Oregon St")
    }

    func testAWashUnderTheStreetIsStillTheWash() throws {
        let press = (lat: 31.7600, lon: -106.4900)
        let fiveNorth = press.lat + 5.0 / 111_320.0
        let index = try XCTUnwrap(tankIndex(lat: fiveNorth, lon: press.lon))
        let card = Inspect.resolve(
            tags: ["highway": "residential", "name": "Oregon St"],
            lat: press.lat,
            lon: press.lon,
        ...
        XCTAssertEqual(card.klass, "STOCK TANK")
        XCTAssertEqual(card.doLine, WaterClass.tank.doLine)
    }

    func testAPaintedDrainTakesTheDrainFieldLineFromTheIndex() throws {
        let index = try XCTUnwrap(WaterIndex.load(try shippedIndexData("tx-west")))
        let card = Inspect.resolve(
            tags: ["waterway": "drain"],
            lat: 31.75959,
        ...
        XCTAssertEqual(card.advice, .leave)
        XCTAssertEqual(card.klass, "DRAIN")
        XCTAssertEqual(card.doLine, WaterClass.drain.doLine)
    }

    /// One unnamed stock tank at a known point, as the wire the phone maps.
    private func tankIndex(lat: Double, lon: Double) -> WaterIndex? {
        var data = Data(Array("BLKTWTR".utf8) + [1])
        for value in [UInt32(1), 1, 1, 0] {
            var le = value.littleEndian
            withUnsafeBytes(of: &le) { data.append(contentsOf: $0) }
        }
        data.append(contentsOf: [
            WaterClass.tank.rawValue,
            WaterEvidence.generic.rawValue,
            13, // natural=water
            1,  // one point
        ])
        for value in [UInt32(0), 0] { // nameAt, then nameLen+pad as two u16 zeros via u32
            var le = value.littleEndian
            withUnsafeBytes(of: &le) { data.append(contentsOf: $0) }
        }
        var first = UInt32(0).littleEndian
        withUnsafeBytes(of: &first) { data.append(contentsOf: $0) }
        var latE7 = Int32(clamping: Int((lat * 1e7).rounded())).littleEndian
        var lonE7 = Int32(clamping: Int((lon * 1e7).rounded())).littleEndian
        withUnsafeBytes(of: &latE7) { data.append(contentsOf: $0) }
        withUnsafeBytes(of: &lonE7) { data.append(contentsOf: $0) }
        return WaterIndex.load(data)
    }
}
