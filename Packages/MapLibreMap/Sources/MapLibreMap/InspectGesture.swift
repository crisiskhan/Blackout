...
public enum InspectGesture {
    /// Same clock the map's hold recogniser uses. A second pair of numbers
    /// would let the tests pass a gesture the phone never runs.
    public static let minimumPressSeconds: Double = Inspect.holdSeconds
    ...
    public static let allowableMovementPoints: Double = Inspect.holdDriftPoints
