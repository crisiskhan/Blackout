...
final class FieldCorpusTests: XCTestCase {
    ...
    func testAskSpokenFieldTalkOpensTheProcedure() {
        ...
        let water = card("water-find", title: "Find water in dry country")
        XCTAssertTrue(FieldCorpus.ask([water], query: "find people", locale: "en").isEmpty)
        XCTAssertTrue(FieldCorpus.ask([water], query: "civilization", locale: "en").isEmpty)
        XCTAssertTrue(FieldCorpus.ask([water], query: "anyone out there", locale: "en").isEmpty)
        XCTAssertEqual(FieldCorpus.ask([water], query: "find water", locale: "en").first?.id, "water-find")
        XCTAssertFalse(FieldCorpus.asking("how do I"))
