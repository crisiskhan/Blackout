...
class PartyPlaceMarkTests(unittest.TestCase):
    ...
    def test_mark_composer_names_notes_and_plants_face(self):
        ...
        self.assertIn("var ink: String", marks)
        self.assertIn("enum MarkInk", marks)
        self.assertIn("case blue = \"BLUE\"", marks)
        ...
        self.assertNotIn("func plantEyeMark", app)
        ...
        self.assertIn("PlaceMarkCard(", tab)
        self.assertIn("Theme.markInk(", tab)
        self.assertIn("runtime.eyeCanvasPips()", tab)
        ...
        self.assertIn('Text("COLOR")', card)
        self.assertIn("MarkInk.allCases", card)
        self.assertLess(card.find('Text("COLOR")'), card.find("EmblemFaceGrid("))
        self.assertNotIn("MarkKind.allCases", card)
        self.assertNotIn('Button("RALLY")', card)
        self.assertNotIn('Button("DOWN")', card)
        self.assertNotIn('Button("WATER")', card)
        ...
        self.assertIn("COLOR", mark_line)
        self.assertIn("SILVER / GREEN / YELLOW / ORANGE / RED / BLUE", mark_line)
        ...
        self.assertIn("NAME the pin on the MARK glass as you plant", qa)
        self.assertIn("pin takes that ink", qa)
        self.assertNotIn("MARK kinds live on the MAP plate", qa)
        ...
        self.assertNotIn("plantEyeMark", commit)
        offline = read("Packages", "MapLibreMap", "Sources", "MapLibreMap", "OfflineMapView.swift")
        self.assertIn("EyeLook.markTint(pip.ink)", offline)
        self.assertIn("static func markTint", offline)
        self.assertIn("parts.count >= 8", mesh)
        self.assertIn("clean(ink)", mesh)
...
class EmblemFaceGridTests(unittest.TestCase):
    ...
    def test_face_lists_scroll_in_look_order_and_stay_compact(self):
        ...
        self.assertIn("minHeight: 0", grid)
        self.assertIn("emblem.title", grid)
...
class MapSearchTests(unittest.TestCase):
    ...
    def test_lookup_is_prefix_folded_and_near_you(self):
        ...
        self.assertIn("testSingleLetterDoesNotAliasEverySouthStreet", tests)
        self.assertIn("testNonFiniteMarkIsNotAHit", tests)
        self.assertIn("testBrokenStreetDocDoesNotTrapTheIndex", tests)
        self.assertIn("31.76190 N, 106.49000 W", tests)
        self.assertIn("31.76190°, -106.49000°", tests)
    ...
    def test_map_types_and_says_and_does_not_invent(self):
        ...
        self.assertNotIn('Button("SAY")', chrome)
        self.assertIn("HUDSearchMic", chrome)
        self.assertIn("reserveTrailing", chrome)
        self.assertIn('Button("PASTE")', chrome)
        self.assertIn("SAY FAILED", chrome)
        self.assertIn("NO PASTE", chrome)
        self.assertIn("UIPasteboard", tab)
        self.assertIn("func keepChrome(", read("Blackout", "AppRuntime.swift"))
        change = tab.split("onChange(of: query)")[1].split("if sayFailed")[0]
        self.assertIn("keepChrome()", change)
        self.assertNotIn("touch(.search)", change)
        self.assertNotIn("pulse()", change)
    ...
    def test_solo_qa_and_device_score_map_search(self):
        ...
        self.assertIn("PASTE", qa)
        self.assertIn("NO PASTE", qa)
        self.assertIn("tear YOU", qa)
        self.assertIn("MAP SEARCH", device)
        self.assertIn("NO MATCH", device)
        self.assertIn("PASTE", device)
        self.assertIn("NO PASTE", device)
        self.assertIn("tear YOU", device)
        self.assertNotIn("best in class", qa.lower())
        self.assertNotIn("best in class", device.lower())
...
class AddressHoldCardTests(unittest.TestCase):
    ...
    def test_search_skips_a_broken_coordinate_instead_of_crashing(self):
        search = read("Packages", "Search", "Sources", "Search", "Search.swift")
        coords = search.split("func coordinates(in")[1].split("public static func rangeLabel")[0]
        self.assertIn("isFinite", coords)
        self.assertIn("rangesByStreet", search)
        overlay = read("Packages", "MapLibreMap", "Sources", "MapLibreMap", "MapLibreMap.swift")
        self.assertIn("func shouldSkipApply", overlay)
        self.assertIn("func boundsSizeChanged", overlay)
        apply = read("Packages", "MapLibreMap", "Sources", "MapLibreMap", "OfflineMapView.swift")
...
class MapCanvasHonestyTests(unittest.TestCase):
    ...
    def test_pinch_stays_on_packed_tiles(self):
        ...
        hold_max = cam.split("static func holdMaxZoom")[1].split("static func allowsOrbit")[0]
    ...
    def test_walking_desk_keeps_the_photo_and_drops_eye_mode(self):
        tokens = read("Packages", "Tokens", "Sources", "Tokens", "Tokens.swift")
        tab = read("Blackout", "MapTab.swift")
        app = read("Blackout", "AppRuntime.swift")
        theme = read("Blackout", "Theme.swift")
        ...
        inst = read("Blackout", "InstrumentsView.swift")
        self.assertNotIn("godsEyeTitle", tokens)
        self.assertNotIn("godsEyeTitle", tab)
        self.assertNotIn("godsEyeTitle", inst)
        self.assertNotIn("toggleGodsEye", tab)
        self.assertNotIn("toggleGodsEye", app)
        self.assertNotIn("HUDOverlayChipStyle(filled: runtime.godsEye)", tab)
        self.assertNotIn("var godsEye", app)
        self.assertNotIn("runtime.godsEye", tab)
        ...
        self.assertNotIn("godsEye", rail)
        self.assertIn("runtime.lockOn", rail)
        self.assertIn("instrumentsTitle", rail)
        self.assertIn("updateTitle", rail)
        canvas = tab.split("private func canvas")[1].split("private var coverUp")[0]
        self.assertNotIn("godsEye", canvas)
        lock = app.split("func toggleLockOn(")[1].split("func ", 1)[0]
        self.assertNotIn("godsEye", lock)
        self.assertIn("EyeDesk.loadLayers()", app)
        ...
        self.assertNotIn('if key == "eye on"', desk)
        self.assertNotIn("khan eye on", desk)
        parse = desk.split("public static func parseVoice")[1].split("public static func hudLine")[0]
        self.assertNotIn('key == "mark water"', parse)
        self.assertIn('hasPrefix("frame ")', parse)
        self.assertNotIn("eye on", parse)
        self.assertNotIn("eye off", parse)
        ...
        live_eye = cam.split("static func liveGodsEye(")[1].split("}")[0]
        self.assertIn("false", live_eye)
        ...
        hold_pitch = cam.split("static func holdPitch(")[1].split("static func followHeading(")[0]
        self.assertIn("return walkPitch", hold_pitch)
        self.assertNotIn("godsEyePitch", hold_pitch)
        hold_min = cam.split("static func holdMinZoom")[1].split("static func holdMaxZoom")[0]
        self.assertIn("return photoMinZoom", hold_min)
        self.assertNotIn("godsEye ? minZoom", hold_min)
        ...
        self.assertIn("godsEye _: Bool", cam.split("static func shouldOpenOnYou(")[1].split("static func shouldFrameDest(")[0])
        self.assertIn("godsEye _: Bool", cam.split("static func shouldFollow(")[1].split("static func shouldFitRoute(")[0])
        self.assertIn("godsEye _: Bool", cam.split("static func shouldFitRoute(")[1].split("public enum PackStyle")[0])
        camera = offline.split("func applyCamera")[1].split("func fitPack")[0]
        ...
        self.assertNotIn("godsEye: runtime.godsEye", tab)
        count = 0
        ...
        self.assertNotIn('sectionLabel("MARK")', inst)
        self.assertNotIn("EyeDesk.MarkKind.allCases", inst)
        self.assertNotIn("plantEyeMark", inst)
        self.assertIn('sectionLabel("MAP")', inst)
        self.assertIn('Button("RULER")', inst)
        ...
        self.assertIn("tap.delegate is Coordinator", interact)
        self.assertIn("tap.isEnabled = false", interact)
        self.assertIn("tap.isEnabled = true", interact)
        self.assertNotIn("tap.isEnabled = godsEye", interact)
        self.assertNotIn("`KHAN EYE`", qa)
        self.assertNotIn("KHAN EYE", qa)
        ...
        self.assertNotIn("EYE drops LOCK-ON", qa)
        self.assertNotIn("never both live", qa)
        ...
        self.assertIn("Walking pinch-in reaches yard zoom", qa)
        ...
        self.assertNotIn("`KHAN EYE`", device)
        self.assertNotIn("KHAN EYE", device)
        self.assertIn("No grey 3D house or tree masses on the photo", device)
        self.assertIn("packed USGS NAIP", device)
        self.assertIn("NAME the pin on the MARK glass as you plant", device)
        self.assertIn("pin takes that ink", device)
        self.assertNotIn("MARK kinds live on the MAP plate", device)
        self.assertIn("Walking pinch-in reaches yard zoom", device)
        self.assertIn("runtime.eyeCanvasPips()", tab)
        self.assertIn("PlaceMark.body", app)
        self.assertIn("PartyBody(", app)
        ...
        self.assertNotIn("onEmptyDoubleTap", tab)
        self.assertNotIn("onPersonDoubleTap", tab)
        self.assertIn("onPersonTap:", tab)
        self.assertIn("holdParty", tab)
        self.assertNotIn("best in class", qa.lower())
        self.assertNotIn("Waze", qa)
        self.assertNotIn("Google", qa)
...
class HUDKeyboardTests(unittest.TestCase):
    ...
    def test_every_field_is_hud_glass_not_uitextfield(self):
        ...
        self.assertIn("reserveTrailing", board)
        self.assertIn("struct HUDSearchMic", board)
        self.assertIn("HUDSearchMicArt", board)
        mic = board.split("struct HUDSearchMic")[1].split("struct HUDKeyboard")[0]
        self.assertIn("Theme.accent", mic)
        self.assertIn("Theme.fix", mic)
        self.assertIn("mapChipHitPoints", mic)
        self.assertNotIn("Color.green", mic)
        self.assertNotIn("systemName", mic)
...
class MapHoldScrollAndPlateRailTests(unittest.TestCase):
    ...
    def test_instruments_is_a_plate_rail_not_one_long_dump(self):
        ...
        self.assertIn('Button("RULER")', mapping)
        self.assertIn('Button("USNG")', mapping)
        self.assertIn('Button("MAG/TRUE")', mapping)
        self.assertNotIn("EyeDesk.noFix", mapping)
        self.assertNotIn("fieldYou == nil", mapping)
        self.assertNotIn('sectionLabel("MARK")', mapping)
        self.assertNotIn("EyeDesk.MarkKind.allCases", mapping)
        self.assertNotIn("plantEyeMark", mapping)
        root = read("Blackout", "RootChrome.swift")
        ...
        self.assertIn("clipped mid-rail", hold_line)
        self.assertIn("sideways", hold_line)
        pages = next(
            line
            for line in qa.splitlines()
            if "glass HUD pages over the still-mounted map" in line
