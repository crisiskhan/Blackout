...
class PartyHoldCardTests(unittest.TestCase):
    ...
    def test_profile_call_and_message_open_comms_for_you_and_party(self):
        ...
        self.assertIn("appendPartyNote", send)
        self.assertIn('"YOU"', send)
        self.assertIn("sendNote", send)
        self.assertIn("var partyNotes", app)
        self.assertIn("var pendingNoteFocus", app)
        self.assertIn("func appendPartyNote(", app)
        self.assertIn("func persistPartyNotes(", app)
        self.assertIn("func loadPartyNotes(", app)
        self.assertIn("persistPartyNotes()", app.split("func appendPartyNote(")[1].split("func callHeldParty(")[0])
        self.assertIn("loadPartyNotes()", app.split("init(")[1].split("func activate(")[0])
        self.assertIn("struct PartyThreadLine", mesh)
        self.assertIn("peer == \"YOU\"", ui)
        self.assertIn('return "YOU"', ui)
        self.assertIn('sectionLabel("THREAD")', comms)
        self.assertIn("partyNotes", comms)
        self.assertIn("pendingNoteFocus", comms)
        self.assertIn("comms.note", comms)
        self.assertIn('Text("NONE")', comms.split('sectionLabel("THREAD")')[1])
        person = next(
            line for line in qa.splitlines() if "Hold YOU or a party emblem" in line
        )
        self.assertIn("YOU and party", person)
        self.assertIn("THREAD", person)
        self.assertIn("local note", person)
