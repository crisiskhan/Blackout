...
class ShippedWaterLayers(unittest.TestCase):
    ...
    def test_every_pack_ships_the_glasshouse_overlay(self):
        ...
        self.assertIn("cherry creek community garden", east_blob)
        ...
        self.assertIn("north open reserve", west_blob)
        self.assertIn("south open reserve", west_blob)
    ...
    def test_a_botanic_garden_is_worked_ground_not_a_meal(self):
        ...
        self.assertEqual(
            ground.overlay_kind(
                {"leisure": "garden", "name": "Cherry Creek Community Garden"}
            ),
            "botanic",
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "residential", "name": "Bayton Loop"})
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "residential", "name": "Cherry Creek Drive"})
        )
        self.assertIsNone(
    ...
    def test_an_open_reserve_is_not_picnic_woodland(self):
        ...
        self.assertIsNone(ground.overlay_kind({"natural": "peak", "name": "Escobas Mesa"}))
        self.assertEqual(
            ground.overlay_kind({"leisure": "park", "name": "Carlito Springs Open Space"}),
            "reserve",
        )
        self.assertEqual(
            ground.overlay_kind({"leisure": "park", "name": "Sedillo Ridge Open Space"}),
            "reserve",
        )
        self.assertIsNone(ground.overlay_kind({"highway": "path", "name": "Juan Trail"}))
        self.assertEqual(
            ground.overlay_kind({"leisure": "park", "name": "Loma Ponderosa Open Space"}),
            "reserve",
        )
        self.assertEqual(
            ground.overlay_kind({"leisure": "park", "name": "Sabino Canyon Open Space"}),
            "reserve",
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "path", "name": "Sabino Canyon East Trail"})
        )
        self.assertEqual(
            ground.overlay_kind(
                {"leisure": "park", "name": "Gutierrez Canyon-Milne Open Space"}
            ),
            "reserve",
        )
        self.assertIsNone(ground.overlay_kind({"highway": "path", "name": "Canyon Trail"}))
        self.assertEqual(
            ground.overlay_kind({"leisure": "park", "name": "Tres Pistolas Open Space"}),
            "reserve",
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "path", "name": "Hawk Watch Connector Spur"})
        )
        self.assertEqual(
            ground.overlay_kind(
                {"leisure": "park", "name": "Ojito de San Antonio Open Space"}
            ),
            "reserve",
        )
        self.assertIsNone(
        ...
        self.assertEqual(
            ground.overlay_kind({"leisure": "nature_reserve", "name": "North Open Reserve"}),
            "reserve",
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "residential", "name": "Ridge Top Drive"})
        )
        self.assertEqual(
            ground.overlay_kind({"leisure": "nature_reserve", "name": "South Open Reserve"}),
            "reserve",
        )
        self.assertIsNone(
            ground.overlay_kind({"highway": "secondary", "name": "Alabama Street"})
        )
        self.assertEqual(
    ...
    def test_the_overlay_and_the_card_use_the_same_cave_preserve_phrases(self):
        ...
        self.assertNotIn('contains("north open")', inspect)
        self.assertNotIn('contains("south open")', inspect)
        self.assertNotIn('contains("cherry")', inspect)
...
class GroundFieldSync(unittest.TestCase):
    ...
    def test_the_button_names_the_procedure_from_the_route(self):
        ...
        self.assertIn("runtime.field.trailBook", tab)
    ...
    def test_a_hold_asks_the_overlay_source_and_the_glass_holds_real_sheets(self):
        ...
        self.assertIn("North Open Reserve", glass)
        self.assertIn("31.839764", glass)
        self.assertIn("-106.459543", glass)
        self.assertIn("Ridge Top Drive", glass)
        self.assertIn("South Open Reserve", glass)
        self.assertIn("31.827637", glass)
        self.assertIn("-106.455905", glass)
        self.assertIn("Alabama Street", glass)
        ...
        self.assertIn("Prosopis velutina / Velvet Mesquite", glass)
        self.assertIn("35.119023", glass)
        self.assertIn("-106.706073", glass)
        ...
        self.assertIn("Cherry Creek Community Garden", glass)
        self.assertIn("30.220468", glass)
        self.assertIn("-97.808122", glass)
        self.assertIn("Bayton Loop", glass)
        self.assertIn("Cherry Creek Drive", glass)
        ...
        self.assertIn("35.138370", glass)
        self.assertIn("-106.481801", glass)
        self.assertIn("High Desert Street Northeast", glass)
        self.assertIn("Foothills Trail North", glass)
        ...
        self.assertIn("Carlito Springs Open Space", glass)
        self.assertIn("35.082960", glass)
        self.assertIn("-106.399414", glass)
        self.assertIn("Sedillo Ridge Open Space", glass)
        self.assertIn("35.045049", glass)
        self.assertIn("-106.300582", glass)
        self.assertIn("Loma Ponderosa Open Space", glass)
        self.assertIn("35.058435", glass)
        self.assertIn("-106.312371", glass)
        self.assertIn("Sabino Canyon Open Space", glass)
        self.assertIn("35.039861", glass)
        self.assertIn("-106.340695", glass)
        self.assertIn("Gutierrez Canyon-Milne Open Space", glass)
        self.assertIn("35.125815", glass)
        self.assertIn("-106.346931", glass)
        self.assertIn("Tres Pistolas Open Space", glass)
        self.assertIn("35.075040", glass)
        self.assertIn("-106.438490", glass)
        ...
        billy_hit = False
        piedmont_hit = False
        north_open_hit = False
        south_open_hit = False
        franklin_hit = False
        lost_dog_hit = False
        flora_hit = False
        jornada_hit = False
            ...
            for ring in rings_of(feat.get("geometry") or {}):
                ...
                if kind == "reserve" and pip(-106.499771, 31.796082, ring):
                    if props.get("name") == "Piedmont City Preserve":
                        piedmont_hit = True
                if kind == "reserve" and pip(-106.459543, 31.839764, ring):
                    if props.get("name") == "North Open Reserve":
                        north_open_hit = True
                if kind == "reserve" and pip(-106.455905, 31.827637, ring):
                    if props.get("name") == "South Open Reserve":
                        south_open_hit = True
        ...
        self.assertTrue(
            north_open_hit,
            "North Open Reserve hold is not inside the named nature reserve",
        )
        self.assertTrue(
            south_open_hit,
            "South Open Reserve hold is not inside the named nature reserve",
        )
        self.assertTrue(
            franklin_hit,
        ...
        for feat in east["features"]:
            ...
            for ring in rings_of(feat.get("geometry") or {}):
                ...
                if kind == "botanic" and name == "Crestview Commons Neighborhood Park" and pip(-97.720514, 30.343843, ring):
                    crestview_hit = True
                if kind == "botanic" and name == "Cherry Creek Community Garden" and pip(-97.808122, 30.220468, ring):
                    cherry_creek_hit = True
        self.assertTrue(
        ...
        self.assertTrue(
            cherry_creek_hit,
            "Cherry Creek Community Garden is not botanic on the east overlay",
        )
        self.assertTrue(
            unity_hit,
        ...
        self.assertIn(
            "Big Brushy Mountain",
            place_names_in_tile("tx-west", -106.518609, 32.598142),
        ...
        for feat in osm["features"]:
            ...
            if (
                props.get("name") == "Big Brushy Mountain"
                and abs(lat - 32.598142) < 1e-6
                and abs(lon - (-106.518609)) < 1e-6
            ):
        ...
        for feat in nm["features"]:
            ...
            if props.get("name") != "Sandia Foothills Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.481801, 35.138370, ring):
        ...
        for feat in nm["features"]:
            ...
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Bandelier National Monument":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.351807, 35.827039, ring):
                    nm_bandelier_hit = True
        self.assertTrue(
            nm_bandelier_hit,
            "Bandelier National Monument hold is not inside the named nature reserve",
        )

        nm_carlito_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Carlito Springs Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.399414, 35.082960, ring):
                    nm_carlito_hit = True
        self.assertTrue(
            nm_carlito_hit,
            "Carlito Springs Open Space hold is not inside the named open space",
        )

        nm_sedillo_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Sedillo Ridge Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.300582, 35.045049, ring):
                    nm_sedillo_hit = True
        self.assertTrue(
            nm_sedillo_hit,
            "Sedillo Ridge Open Space hold is not inside the named open space",
        )

        nm_ponderosa_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Loma Ponderosa Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.312371, 35.058435, ring):
                    nm_ponderosa_hit = True
        self.assertTrue(
            nm_ponderosa_hit,
            "Loma Ponderosa Open Space hold is not inside the named open space",
        )
        ...
        for feat in nm["features"]:
            ...
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Sabino Canyon Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.340695, 35.039861, ring):
                    nm_sabino_hit = True
        self.assertTrue(
            nm_sabino_hit,
            "Sabino Canyon Open Space hold is not inside the named open space",
        )

        nm_gutierrez_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Gutierrez Canyon-Milne Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.346931, 35.125815, ring):
                    nm_gutierrez_hit = True
        self.assertTrue(
            nm_gutierrez_hit,
            "Gutierrez Canyon-Milne Open Space hold is not inside the named open space",
        )

        nm_pistolas_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Tres Pistolas Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.438490, 35.075040, ring):
                    nm_pistolas_hit = True
        self.assertTrue(
            nm_pistolas_hit,
            "Tres Pistolas Open Space hold is not inside the named open space",
        )

        nm_ojito_san_hit = False
        for feat in nm["features"]:
            props = feat.get("properties") or {}
            if ground.overlay_kind(props) != "reserve":
                continue
            if props.get("name") != "Ojito de San Antonio Open Space":
                continue
            for ring in rings_of(feat.get("geometry") or {}):
                if pip(-106.386934, 35.106397, ring):
                    nm_ojito_san_hit = True
        self.assertTrue(
            nm_ojito_san_hit,
    ...
    def test_solo_qa_scores_the_biome_handoff(self):
        ...
        self.assertIn("Organ Mountains-Desert Peaks National Monument", qa)
        self.assertIn("32.182443", qa)
        ...
        self.assertIn("35.138370", qa)
        self.assertIn("High Desert Street Northeast", qa)
        self.assertIn("Foothills Trail North", qa)
        ...
        self.assertIn("35.082960", qa)
        self.assertIn("Sedillo Ridge Open Space", qa)
        self.assertIn("35.045049", qa)
        self.assertIn("Loma Ponderosa Open Space", qa)
        self.assertIn("35.058435", qa)
        self.assertIn("Sabino Canyon Open Space", qa)
        self.assertIn("35.039861", qa)
        self.assertIn("Gutierrez Canyon-Milne Open Space", qa)
        self.assertIn("35.125815", qa)
        ...
        self.assertIn("North Open Reserve", qa)
        self.assertIn("31.839764", qa)
        self.assertIn("Ridge Top Drive", qa)
        self.assertIn("South Open Reserve", qa)
        self.assertIn("31.827637", qa)
        self.assertIn("Alabama Street", qa)
        ...
        self.assertIn("Cherry Creek Community Garden", qa)
        self.assertIn("30.220468", qa)
        self.assertIn("Bayton Loop", qa)
        self.assertIn("Cherry Creek Drive", qa)
    ...
    def test_the_state_book_names_the_vision_species_as_range(self):
        ...
        self.assertIn("runtime.field.trail", tab)
        self.assertIn("leaveCard()", tab)
        self.assertIn("advanceTrail()", tab)
        self.assertNotIn('Button(s.isLast ? "DONE" : "NEXT")', tab)
        self.assertIn("runtime.field.trailTotal", tab)
        self.assertIn('L10n.t("field.card"', tab)
        field = SWIFT.read_text()
        ...
        self.assertIn("case .fire:", vision_fn)
        self.assertIn("case .flood:", vision_fn)
        self.assertIn("case .ice:", vision_fn)
        self.assertIn("case .smoke:", vision_fn)
        self.assertIn("case .lightning:", vision_fn)
        self.assertIn("case .shelter:", vision_fn)
        self.assertIn("case .wound:", vision_fn)
        self.assertIn("env-wildfire", vision_fn)
        self.assertIn("env-flood", vision_fn)
