...
class ZoomGates(unittest.TestCase):
    ...
    def test_the_lines_wait_for_the_zoom_the_tiles_carry_them_at(self):
        for pid in PACKS:
            ...
            self.assertEqual(line.get("minzoom"), water.LINE_MIN_ZOOM, pid)

    def test_the_class_marks_and_then_their_names_come_in_close(self):
        for pid in PACKS:
            style = style_of(pid)
            points = layer(style, "water-detail-points")
            labels = layer(style, "water-detail-labels")
            self.assertIsNotNone(points, pid)
            self.assertIsNotNone(labels, pid)
            self.assertEqual(points.get("minzoom"), water.DETAIL_MIN_ZOOM, pid)
            self.assertEqual(labels.get("minzoom"), water.LABEL_MIN_ZOOM, pid)
    ...
    def test_the_marks_read_the_shipped_file_and_nothing_remote(self):
        for pid in PACKS:
            style = style_of(pid)
            source = style["sources"]["water-detail"]
            self.assertEqual(source["type"], "geojson")
            self.assertEqual(source["data"], "layers/water.geojson")
            for layer_id in ("water-detail-points", "water-detail-labels"):
                entry = layer(style, layer_id)
                self.assertEqual(entry["source"], "water-detail", pid)
                # geojson sources are not tiled, so no source-layer is wanted
                # and the vector-source guard does not apply to them.
                self.assertNotIn("source-layer", entry, f"{pid} {layer_id}")
            self.assertNotIn("http", json.dumps(style["sources"]["water-detail"]))

    def test_the_marks_are_a_dot_and_a_word_rather_than_an_icon(self):
        # No sprite, no icon-image: the palette stays void/red/silver plus the
        # water ink the style already had.
        for pid in PACKS:
            style = style_of(pid)
            self.assertNotIn("sprite", style)
            labels = layer(style, "water-detail-labels")
            self.assertNotIn("icon-image", labels["layout"])
            self.assertEqual(labels["layout"]["text-font"], ["Open Sans Regular"])
            self.assertEqual(labels["paint"]["text-color"], "#B8BDC2")
            points = layer(style, "water-detail-points")
            self.assertEqual(points["type"], "circle")
            self.assertEqual(points["paint"]["circle-stroke-color"], "#B8BDC2")
...
class HoldToInspect(unittest.TestCase):
    def test_a_press_is_a_press_and_a_drag_is_a_pan(self):
        gesture = GESTURE_SWIFT.read_text()
        seconds = float(re.search(r"minimumPressSeconds: Double = ([\d.]+)", gesture).group(1))
        slop = float(re.search(r"allowableMovementPoints: Double = ([\d.]+)", gesture).group(1))
        self.assertTrue(0.3 < seconds < 0.8, seconds)
        self.assertTrue(0 < slop <= 16, slop)
        offline = OFFLINE_SWIFT.read_text()
        # The recognizer has to be built from the same numbers the policy is
        # tested against, or the tests are describing something else.
        self.assertIn("press.minimumPressDuration = InspectGesture.minimumPressSeconds", offline)
        self.assertIn(
            "press.allowableMovement = CGFloat(InspectGesture.allowableMovementPoints)",
            offline,
        )
        self.assertIn("guard gesture.state == .began", offline)

    def test_the_press_swallows_the_tap_that_ends_the_same_touch(self):
        offline = OFFLINE_SWIFT.read_text()
        self.assertIn("guard !pressBeganInspect else { return }", offline)
        self.assertIn("pressBeganInspect = false", offline)

    def test_the_card_is_half_the_glass_at_most_and_lifts_the_pin_clear(self):
        gesture = GESTURE_SWIFT.read_text()
        self.assertIn("maxHeightFraction: Double = 0.5", gesture)
        tab = MAP_TAB.read_text()
        self.assertIn("InspectCard.maxHeight(screenHeight:", tab)
        self.assertIn("inspectCardHeight: cardHeight", tab)
        self.assertIn("liftForCard", OFFLINE_SWIFT.read_text())

    def test_the_card_says_what_sure_is_not(self):
        swift = SWIFT.read_text()
        said = re.search(r'disclaimer =\s*"([^"]+)"', swift).group(1).lower()
        self.assertIn("record", said)
        self.assertIn("drink", said)
        self.assertIn("WaterSure.disclaimer", CARD.read_text())

    def test_the_card_carries_class_sure_why_do_field_and_mark(self):
        card = CARD.read_text()
        self.assertIn("finding.title", card)
    ...
    def test_a_press_on_land_opens_the_matching_field_stepper(self):
        swift = SWIFT.read_text()
        self.assertIn('public static let land = "nav-lost"', swift)
        self.assertIn('public static let water = "water-disinfect"', swift)
        ...
        self.assertIn("runtime.pendingFieldCardID", field)
        self.assertIn("StepperState(card: card, index: 0", field)

    def test_sos_stays_on_comms(self):
        tokens = TOKENS_SWIFT.read_text()
        block = tokens.split("func sosFAB")[1].split("}\n    }")[0]
        self.assertIn("case .comms:\n                return true", block)
        self.assertIn("case .map, .field, .expedition:\n                return false", block)
    ...
    def test_the_press_reads_the_pack_on_disk_and_nothing_else(self):
        for path in (SWIFT, GESTURE_SWIFT, CARD, MAP_TAB):
            text = path.read_text()
            for banned in ("URLSession", "http://", "https://", "MapKit", "CoreML", "Vision"):
    ...
    def test_the_map_still_draws_what_it_already_proved(self):
        offline = OFFLINE_SWIFT.read_text()
        # Cyan walk line, YOU puck, destination pin, OSM credit.
