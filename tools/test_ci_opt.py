...
def test_pbx_single_copy() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 39" not in pbx:
        fail("CURRENT_PROJECT_VERSION is no longer 39")
    if pbx.count("CURRENT_PROJECT_VERSION = 39") < 2:
        fail("expected CURRENT_PROJECT_VERSION = 39 on Debug and Release")
...
def test_generator_does_not_restore_double_copy() -> None:
    ...
    if '"CURRENT_PROJECT_VERSION": "39",' not in src:
...
def test_live_mesh_1n() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 39" not in pbx:
        fail("version was bumped off 39")
    if "MARKETING_VERSION = 0.1.0" not in pbx:
...
def test_bundled_statewide_archive_only() -> None:
    ...
    if pbx.count("CURRENT_PROJECT_VERSION = 39") < 2:
        fail("CURRENT_PROJECT_VERSION was bumped off 39")
...
def pbx_version_off_32() -> bool:
    pbx = (ROOT / "Blackout.xcodeproj/project.pbxproj").read_text()
    return pbx.count("CURRENT_PROJECT_VERSION = 39") < 2
...
def test_locked_app_icon() -> None:
    ...
    if "CURRENT_PROJECT_VERSION" in (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text():
        pbx = (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text()
        if pbx.count("CURRENT_PROJECT_VERSION = 39") < 2:
            fail("version was bumped off 39 while landing the emblem")
...
def test_sos_armed_restore_no_crash() -> None:
    ...
    if pbx.count("CURRENT_PROJECT_VERSION = 39") < 2:
        fail("do not bump CURRENT_PROJECT_VERSION")
    ok("SOS armed restore + lockup first-open + launch crash sweep, version 39")
...
def test_map_fill_bleed_and_paint_budget() -> None:
    ...
    if pbx.count("CURRENT_PROJECT_VERSION = 39") < 2:
        fail("do not bump CURRENT_PROJECT_VERSION")
    ok("Map canvas fills, streets/topo session-off, paint budget cut, version 39")
