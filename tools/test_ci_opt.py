...
def test_pbx_single_copy() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 32" not in pbx:
        fail("CURRENT_PROJECT_VERSION is no longer 32")
    if pbx.count("CURRENT_PROJECT_VERSION = 32") < 2:
        fail("expected CURRENT_PROJECT_VERSION = 32 on Debug and Release")
...
def test_generator_does_not_restore_double_copy() -> None:
    ...
    if '"CURRENT_PROJECT_VERSION": "32",' not in src:
...
def test_live_mesh_1n() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 32" not in pbx:
        fail("version was bumped off 32")
    if "MARKETING_VERSION = 0.1.0" not in pbx:
...
def test_locked_app_icon() -> None:
    ...
    if "CURRENT_PROJECT_VERSION" in (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text():
        pbx = (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text()
        if pbx.count("CURRENT_PROJECT_VERSION = 32") < 2:
            fail("version was bumped off 32 while landing the emblem")
...
def test_sos_armed_restore_no_crash() -> None:
    ...
    ok("SOS armed restore + lockup first-open + launch crash sweep, version 32")
