...
def test_pbx_single_copy() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 31" not in pbx:
        fail("CURRENT_PROJECT_VERSION is no longer 31")
    if pbx.count("CURRENT_PROJECT_VERSION = 31") < 2:
        fail("expected CURRENT_PROJECT_VERSION = 31 on Debug and Release")
...
def test_generator_does_not_restore_double_copy() -> None:
    ...
    if '"CURRENT_PROJECT_VERSION": "31",' not in src:
...
def test_live_mesh_1n() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 31" not in pbx:
        fail("version was bumped off 31")
    if "MARKETING_VERSION = 0.1.0" not in pbx:
...
def test_bundled_statewide_archive_only() -> None:
    ...
    if pbx.count("CURRENT_PROJECT_VERSION = 31") < 2:
        fail("CURRENT_PROJECT_VERSION was bumped off 31")
...
def test_locked_app_icon() -> None:
    ...
    if "CURRENT_PROJECT_VERSION" in (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text():
        pbx = (ROOT / "Blackout.xcodeproj" / "project.pbxproj").read_text()
        if pbx.count("CURRENT_PROJECT_VERSION = 31") < 2:
            fail("version was bumped off 31 while landing the emblem")
