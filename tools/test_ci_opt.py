...
def test_pbx_single_copy() -> None:
    ...
    if "CURRENT_PROJECT_VERSION = 15" not in pbx:
        fail("CURRENT_PROJECT_VERSION is no longer 15")
    if pbx.count("CURRENT_PROJECT_VERSION = 15") < 2:
        fail("expected CURRENT_PROJECT_VERSION = 15 on Debug and Release")
    if "MARKETING_VERSION = 0.1.0" not in pbx:
        fail("MARKETING_VERSION is no longer 0.1.0")
    ok("pbxproj is ditto-only, version 15, no alwaysOutOfDate")
...
def test_generator_does_not_restore_double_copy() -> None:
    ...
    if '"CURRENT_PROJECT_VERSION": "15"' not in src:
    ...
    ok("generate_project.py regen stays ditto-only at version 15")
